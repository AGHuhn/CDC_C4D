...
class AppWindow(QMainWindow, Ui_MainWindow):
    ...
    def set_plot(self, reaktivate = False):
        ...
        for index in range(self.am_cdc):
            ...
            self.ax[index].set_ylabel('Capacitance /fF')
    ...
    def file_save(self):
        ...
        for i in range(0,self.am_cdc):
            ...
            sf=open(temp_name, 'w')
            sf.write(str(self.headTempFile+ str(cdcNo) + ' /fF\n'))
    ...
    def file_control(self, way):
        if way in ['w', 'a'] :
            ...
            dateTemp = time.strftime("%Y-%m-%d_%H-%M_%S") #save date for temp data
            for i in range(0,self.am_cdc):
                cdcNo = i+1;
                ...
                if way == 'w':
                    self.d[i].write(str(self.headTempFile+ str(cdcNo) + ' /fF\n'))
        if way == 'c':
            self.tempfiles = []
            for i in range(0,self.am_cdc):
                self.d[i].close()
